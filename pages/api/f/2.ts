...
const TARGET = "https://exam-server.hanbin123.com/api/v1/cli/subjects/ai_trainer/materials";
...
function formatAsText(data: { items: Array<{ title: string; content: string }> }): string {
  return data.items
    .map((item) => `# ${item.title}\n\n${item.content}`)
    .join("\n\n---\n\n");
}
...
export default async function handler(
  ...
  try {
    ...
    const responseData = result.data as { code: number; data: { items: Array<{ title: string; content: string }> } };
